...
def load_indeed_jobs_div(job_title, location):
    getVars = {'q' : job_title, 'fromage' : 'last', 'sort' : 'date'}
    url = ('http://sg.indeed.com/jobs?' + urllib.parse.urlencode(getVars))
...
def extract_job_information_indeed(job_soup, desired_characs):
    ...
    if 'description' in desired_characs:
        ...
        for job_elem in job_elems:
            description.append(extract_description(job_elem))
...
def extract_job_title_indeed(job_elem):
    title_elem = job_elem.find("h2", "jobTitle")
...
def extract_description(job_elem):
    description_elem = job_elem.find(class_="job-snippet")
...
find_jobs_from("Indeed", "python", "Singapore", desired_characs = ['titles', 'companies', 'links', 'date_listed', 'description'], filename="results.xls")